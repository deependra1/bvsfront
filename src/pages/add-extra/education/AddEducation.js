...
export default function AddEducation() {
  ...
  const handleEdit = (education) => {
    setSelectedEducation(education.row);
    setOpen(true);
  };

  const handleDelete = (id) => {
    if (confirm('Are you sure to delete education level?')) {
      axiosService
        .delete(`/educationlevel/${id}/`)
        .then(() => {
  ...
  const handleAddEducation = (values, { setErrors, setStatus, setSubmitting }) => {
    if (selectedEducation.id) {
      axiosService
        .put(`/educationlevel/${selectedEducation.id}/`, values)
        .then(() => {
    ...
    <div>
      ...
      <BootstrapDialog onClose={handleClose} aria-labelledby="customized-dialog-title" open={open}>
        <BootstrapDialogTitle id="customized-dialog-title" onClose={handleClose}>
          {selectedEducation.id ? 'Update Education Level' : 'Add Education Level'}
            ...
            <Form noValidate>
              <DialogContent dividers>
                <Grid container spacing={3}>
                  ...
                  <Grid item xs={12}>
                    <Stack spacing={1}>
                      <InputLabel htmlFor="ethnic">Education Level*</InputLabel>
                      <OutlinedInput
                        fullWidth
                        id="ethnic"
                        type="text"
                        ...
                        error={Boolean(touched.education_level && errors.education_level)}
                      />
                      {touched.education_level && errors.education_level && (
                        <FormHelperText error id="standard-weight-helper-text-education_level">
              ...
              <DialogActions>
                <Button autoFocus variant="outlined" disableElevation disabled={isSubmitting} onClick={handleSubmit} color="primary">
                  {selectedEducation.id ? 'Update' : 'Save'}
                </Button>
