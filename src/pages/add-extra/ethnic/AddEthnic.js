...
export default function AddEthnic() {
  const [open, setOpen] = React.useState(false);
  const { enqueueSnackbar } = useSnackbar();
  const [selectedEthnic, setSelectedEthnic] = React.useState({});

  // fetching treatment data
  const {
    data: ethnicData,
    error: ethnicError,
    isLoading: ethnicLoading,
    mutate: ethnicMutate
  } = useSWR(`/ethnic/`, fetcher, { revalidateOnMount: true });
  // end of fetching
  ...
  const handleEdit = (ethnic) => {
    setSelectedEthnic(ethnic.row);
    setOpen(true);
  };

  const handleDelete = (id) => {
    if (confirm('Are you sure to delete ethnic group?')) {
      axiosService
        .delete(`/ethnic/${id}/`)
        .then(() => {
          enqueueSnackbar('Ethnic Group Deleted successfully!!!', { variant: 'success' });
          ethnicMutate();
        })
        .catch(() => {
  ...
  const handleAddEthnic = (values, { setErrors, setStatus, setSubmitting }) => {
    if (selectedEthnic.id) {
      axiosService
        .put(`/ethnic/${selectedEthnic.id}/`, values)
        .then(() => {
      ...
      axiosService
        .post(`/ethnic/`, values)
        .then(() => {
          enqueueSnackbar('Ethnic group added successfully!!!', { variant: 'success' });
          setStatus({ success: false });
          setSubmitting(false);
          ethnicMutate();
        })
        .catch((err) => {
          enqueueSnackbar('Something went wrong while adding the ethnic group!!!', { variant: 'error' });
          setStatus({ success: false });
          setErrors({ submit: err.message });
          setSubmitting(false);
        });
    ...
    <div>
      ...
      <BootstrapDialog onClose={handleClose} aria-labelledby="customized-dialog-title" open={open}>
        <BootstrapDialogTitle id="customized-dialog-title" onClose={handleClose}>
          {selectedEthnic.id ? 'Update Ethnic Group' : 'Add Ethnic Group'}
        </BootstrapDialogTitle>
        <Formik
          enableReinitialize={true}
          initialValues={{
            ethnic_group: selectedEthnic?.ethnic_group || ''
          }}
          validationSchema={Yup.object().shape({
            ethnic_group: Yup.string().required('Ethnic group is required')
          })}
            ...
            <Form noValidate>
              <DialogContent dividers>
                <Grid container spacing={3}>
                  ...
                  <Grid item xs={12}>
                    <Stack spacing={1}>
                      <InputLabel htmlFor="ethnic">Ethnic Group*</InputLabel>
                      <OutlinedInput
                        fullWidth
                        id="ethnic"
                        type="text"
                      ...
                      />
                      {touched.ethnic_group && errors.ethnic_group && (
